...
movies.forEach(movie => {
  movies.forEach(movie => {
    let movieContent = "" +
      `<div class="movie" id="card${movie.id}">`+
        `<a href="${movie.vikipediaLink}" target="_blank"/><img src="${movie.imgUrl}"/></a>`;
        if(movie.name )
          movieContent +=`<h2>${movie.name}</h2>`;
        if(movie.year)
          movieContent +=`<h3>${movie.year}</h3>`;
        if(movie.rating)
          movieContent +=`<h4>${movie.rating}</h4>`;
        if(movie.imdb || movie.imdb===0)
          movieContent +=`<h4>${movie.imdb}</h4>`;
        movieContent+= `</div>`;
